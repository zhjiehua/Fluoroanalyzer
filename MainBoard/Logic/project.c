...
//			ResetDevice();//复位串口屏
	...
	DDC112IF_IntegralTimeSetting(19); //14*500us  25
	DDC112IF_BufferSizeSetting(3); //2
...
//					SetScreen(TIPS1PAGE_INDEX);
				...
				else if(abs(pProjectMan->axisCXCur - pProjectMan->axisCX) > pProjectMan->checkWindowAxisX)
				{
					//
					pProjectMan->gender = 0;
				...
				{
					temp = pProjectMan->axisCXCur + pProjectMan->axisTX - pProjectMan->axisCX - pProjectMan->checkWindowAxisX/2;
					pProjectMan->axisTXCur = FindMaximin(pProjectMan->sampleData+temp, pProjectMan->checkWindowAxisX); //找第二个极大点
					pProjectMan->axisTXCur += temp;
					pProjectMan->tThreshod = pProjectMan->LeastSquareA*pProjectMan->axisTXCur + pProjectMan->LeastSquareB;
